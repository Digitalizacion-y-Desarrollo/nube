...
<nav aria-label="Navegación inferior" class="fixed inset-x-0 bottom-0 z-40 border-t border-[#e5e7eb] bg-white/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1 backdrop-blur-lg lg:hidden">
    <div class="mx-auto flex h-16 max-w-md items-center justify-between">
        @foreach ($items as $item)
            <a href="{{ route('dashboard') }}" @class([
                ...
                'font-semibold text-brand' => $item['active'] ?? false,
                'font-medium text-[#6b7280]' => ! ($item['active'] ?? false),
            ])>
